...
export default function KioskOrderPage() {
  ...
  const profile = JSON.parse(sessionStorage.getItem("kiosk_profile") || "null");
  ...
  const [preferredStaffByArea, setPreferredStaffByArea] = useState({}); // areaId -> staffId
  ...
  <div className={styles.wrap}>
    <div className={styles.shell}>
      <div className={styles.header}>
        ...
        <button className={styles.back} onClick={() => nav("/register")} type="button">
          Zurück
        </button>
      </div>
...
function TreatmentPanel({ categories, services, selectedServiceIds, onToggleService }) {
  ...
  useEffect(() => { if (!cat && categories?.[0]) setCat(categories[0]); }, [categories, cat]);

  const visible = services.filter(s => s.category === cat);
    ...
    <div className={styles.panel}>
      <div className={styles.panelHead}>
        ...
        <div className={styles.pillsRow}>
          {categories.map((c) => (
            <button key={c} className={`${styles.pill} ${c === cat ? styles.pillOn : ""}`} onClick={() => setCat(c)} type="button">
              {c}
            </button>
          ))}
        </div>
          ...
          {requestedAreaIds.map((areaId) => (
            <div key={areaId} className={styles.staffCard}>
              ...
              <div className={styles.small}>
                Wenn gesetzt, erscheint der Kunde direkt in der Warteliste dieses Mitarbeiters (Step-2 Feinsortierung).
              </div>
            </div>
          ))}
        </div>
...
function NotesPanel({ value, onChange }) {
  return (
    <div className={styles.panel}>
      ...
      <textarea className={styles.textarea} value={value} onChange={(e) => onChange(e.target.value)} placeholder="Kommentar / Hinweis..." />
    </div>
  );
}
          ...
          {products.map(p => (
            <div key={p.id} className={styles.item}>
              <div>
                ...
                <div className={styles.itemMeta}>{p.brand} · {Number(p.price).toFixed(2)} €</div>
              </div>
              <button className={styles.itemBtn} onClick={() => onAdd(p)} type="button">+ Add</button>
            </div>
          ))}
        </div>
            ...
            cart.map(x => (
              <div key={x.id} className={styles.cartRow}>
                ...
                <div className={styles.cartQty}>
                  <button className={styles.qtyBtn} onClick={() => onDec(x.id)} type="button">-</button>
                  <span>x{x.qty}</span>
                </div>
